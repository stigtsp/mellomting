...
// whitespace.
//
// A name the model ACL would read as a sentinel is refused here as well
// as at configuration validation: the server answering this endpoint is
// not trusted yet.
func validModelID(s string) bool {
	if s == "" || len(s) > MaxModelIDBytes || !utf8.ValidString(s) || config.ValidateModelName(s) != nil {
		return false
	}
	for _, r := range s {
